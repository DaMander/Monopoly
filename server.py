...
class Server():
    ...
    def player_rolled_dice(self, roll: list, pl_list: list):
        ...
        if pl_list[self.turn] in self.jail_list and self.double is False:#if the player is in jail or they have not rolled a double then they can not move
            ...
            if self.check_for_triple():#if triple_checker == 3 returns True
    ...
    def pay_tax_or_property_rent(self, pl_list: list, roll):#when player lands on an owned property or tax card it will display the card and money they own
        self.action_time = time.time() if self.action_time == 0 else self.action_time
        if time.time() - self.action_time >= 3.5:#it will keep it at the current action taken value for 1.5 seconds, the property and rent amount will be displayed
            if self.action_taken == 7:#this means a player has landed on an owned property that another player owns
                self.have_enough_money, self.amount_required = pl_list[self.turn].pay_rent(self.board, roll)#checks whether the player can pay the amount, if not then have_enough_money becomes False
    ...
    def player_move(self, data: str, player_id: int, pl_list:list, player_squares: list, conn):
        ...
        if self.turn == -1:
            ...
            if pl_list[self.turn] == self.players[player_id]:
            ...
            self.pay_tax_or_property_rent(pl_list, dice_roll()[0])
    ...
    def compensate_game(self, player_id):
    ...
    def disconnect_player(self, player_id, pl_list, conn):#if a player disconnects they need to be declared out in the game and then carry on with the next players turn
        ...
        if len(self.players) - self.calculate_players_out() > 0 and self.turn != -1:#changes turn unless the game has finished
    ...
    def threaded_client(self, conn, id: int):#main function where the players will have their own thread and play the game
        ...
        if __name__ == "__main__":
            ...
            self.compensate_game(player_id)
            ...
            if not self.players[player_id].out:
...
while True:
    ...
    if server.connections == 0:
        server = Server()
    if server.turn == -1 and len(server.players) < 6:
